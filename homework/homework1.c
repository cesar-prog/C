#include <stdio.h>  /* greetings to rulegamer and satuelissa */
#include <string.h> // strstr

#define MAX 150
void quitamq(char linea[MAX], char lineasin[MAX]); /* Function that eliminate < >  */
...
int main() {
  ...
  while ((l = saca()) == 0) {
    ...
    if (strstr(linea, "<h1") != NULL || strstr(linea, "<h2") != NULL || strstr(linea, "<h3") != NULL ||  strstr(linea, "<h4") != NULL || strstr(linea, "<h5") != NULL  || strstr(linea, "<h6") != NULL  ) {
      quitae(linea,lineasin);
      quitamq(lineasin, lineasinetiquetas);
      printf("%s\n", lineasinetiquetas);
    }
    else if (strstr(linea, "<strong>") != NULL) {
      strong(linea, lineasinstrong);
      quitamq(lineasinstrong,lineasinetiquetas);
      printf("%s\n", lineasinetiquetas);
    }

      else{
      quitamq(linea,lineasinetiquetas);
      printf("%s\n", lineasinetiquetas);
    }
    }
    if (strstr(linea, "</script>") != NULL) { // "</script>" stop ignoring
      ignore = 0;
    }
    if (strstr(linea, "</div>") != NULL) {
      ignore = 0;
...
void quitae(char cadena [MAX], char cadenasin[MAX])
{
int a;
...
void quitamq(char cadena[MAX], char cadenasin[MAX]){
int a;
int c=0;
int b=0;
for (a=0;a<MAX;a++){
